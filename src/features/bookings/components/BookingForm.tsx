...
const BookingForm = ({ listingId, onClose }: Props) => {
  // Removed unused `defaultPersonal` from destructure
  const { currentStep, data, next, back, submit, isSubmitting, isSuccess, error } = useBooking(listingId);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="w-full max-w-lg rounded-2xl shadow-2xl p-6 relative" style={{ backgroundColor: "#f7f3ef" }}>
            ...
            <div key={label} className="flex items-center gap-2 flex-1">
              <div className="flex flex-col items-center">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
                    i <= currentStep ? "text-white" : "bg-slate-200 text-slate-400"
                  }`}
              ...
              {i < STEPS.length - 1 && (
                <div
                  ...
                  style={{ backgroundColor: i < currentStep ? P : "#e2e8f0" }}
                />
              )}
            </div>
          ))}
        ...
        {isSuccess && (
          <button
            onClick={onClose}
            className="mt-4 w-full py-2.5 text-white font-semibold rounded-xl hover:opacity-90 transition-opacity"
