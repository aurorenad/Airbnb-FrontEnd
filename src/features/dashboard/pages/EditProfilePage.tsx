...
const EditProfilePage = () => {
  ...
  useLayoutEffect(() => {
    ...
    // Fix: avatar can be undefined — coerce to null
  ...
  const inputClass =
    "w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 dark:bg-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-orange-300 transition";
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="rounded-2xl border border-black/5 shadow-sm p-6" style={{ backgroundColor: BG }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(["facebook", "twitter", "instagram", "linkedin"] as const).map((field) => (
              <div key={field}>
                <label className="block text-xs font-medium text-slate-600 mb-1 capitalize">
                  {field} <span className="text-slate-400">(optional)</span>
                </label>
                <input
                  type="url"
                  value={form[field]}
