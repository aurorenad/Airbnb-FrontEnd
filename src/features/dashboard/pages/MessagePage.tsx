...
const MessagePage = () => {
  ...
  const rawActiveId = params.get("user");
  const activeId = rawActiveId === user?.id ? null : rawActiveId;
  ...
  const activePartner = useMemo(() => {
    ...
    const conversation = conversations.find((c) => c.partner.id === activeId);
    if (conversation) return conversation.partner;
    ...
    return message.senderId === activeId ? message.sender : message.receiver;
  }, [activeId, conversations, messagesQuery.data]);

  // Derived — no state needed
  const mobileView: "list" | "chat" = activeId ? "chat" : "list";

  return (
    <div
      className="flex h-[calc(100vh-5rem)] rounded-lg overflow-hidden border border-black/5 shadow-sm"
      style={{ backgroundColor: BG }}
    >
      {/* ── Conversation list ── */}
      <div
        className={`${mobileView === "chat" ? "hidden" : "flex"} md:flex flex-col w-full md:w-80 border-r border-black/10 shrink-0`}
        style={{ backgroundColor: BG }}
      >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex justify-between items-center mt-0.5">
                  ...
                  {conversation.unread > 0 && (
                    <span
                      className="text-[10px] font-bold text-white rounded-full min-w-4 h-4 px-1 flex items-center justify-center shrink-0 ml-1"
      ...
      {activeId ? (
        ...
        >
          {/* Header */}
          <div
            className="flex items-center gap-3 px-4 py-3 border-b border-black/10"
            style={{ backgroundColor: BG }}
          >
            <button
              onClick={() => setParams({})}
            ...
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm text-slate-900">
                {activePartner?.name ?? "New conversation"}
              </p>
          ...
          {/* Input */}
          <div
            className="px-4 py-3 border-t border-black/10 flex items-center gap-3"
            style={{ backgroundColor: BG }}
          >
            <input
              value={input}
              ...
              onKeyDown={(e) =>
                e.key === "Enter" && input.trim() && sendMutation.mutate()
              }
